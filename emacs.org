...
#+begin_src emacs-lisp
	...
	(setq holiday-local-holidays
				'((holiday-fixed 1 1 "New Years Day")
					...
					(fc/filtered-holidays fc/holidays-2022)
					(fc/filtered-holidays fc/holidays-2023)
					))
#+end_src
...
#+begin_src emacs-lisp
	(use-package yasnippet
		:ensure t
		:init
		(diminish 'yas-minor-mode '(:propertize " yas" face '(:foreground "coral")))
...
#+begin_src emacs-lisp
	;; base directories
	(setq
	 dotfiles-home (expand-file-name "dotfiles" (getenv "HOME"))
	 macbook-home (expand-file-name "shyam/macbook" (getenv "HOME"))
	 projects-home (expand-file-name "projects" (getenv "HOME"))
	 )

	(defun fc/project-matcher (path)
		"matcher to find org files"
		(string-suffix-p "-notes.org" path)
		)
	(defun fc/make-pair (path)
		"create a pair with the directory name & filename"
		(cons (f-base (f-dirname path)) path))
				...
				(let ((remove-prefix
							 (lambda (x)
								 (replace-regexp-in-string
									...
									""
									x))))
					(seq-map #'fc/make-pair
									 (seq-map remove-prefix
														(f-files projects-home #'fc/project-matcher t)))))
	;; sort ?
	...
	(defun fc/daily-make-pair (path)
		...
		(cons (replace-regexp-in-string "daily-log-" "" (f-base path)) path))
	...
	(setq daily-notes
				(let ((remove-prefix
					...
					(seq-map #'fc/daily-make-pair
									 (seq-map remove-prefix
														(f-files macbook-home #'fc/daily-notes-matcher t)))))
...
#+begin_src emacs-lisp
	...
	(defhydra common-org-files (:exit t)
		...
		("d" (find-file (expand-file-name "daily-log-2023.org" macbook-home)) "daily-log-2023.org")
...
*** Plantuml jar path
#+begin_example emacs-lisp
	(defun fc/unbrew (src target)
		"Expand the symbolic link to the brew target"
		(let* ((brew-prefix "/usr/local")
					 (plantuml-prefix (file-chase-links (format "%s/%s" brew-prefix src))))
...
#+begin_src emacs-lisp
	...
	(mapc (lambda (hook) (add-hook hook #'fc/add-yas-minor-mode)) yas-modes)
#+end_src
* Mode customizations
