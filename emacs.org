...
#+begin_src emacs-lisp
	(use-package web-mode
		:mode "\\.\(phtml\|tpl\\.php\|[agj]sp\|as[cp]x\|erb\|mustache\|hbs\|djhtml\)\\'"
		:custom
...
#+begin_src emacs-lisp
	...
	(setq project-notes
				(let ((remove-prefix
							 ;; remove the projects-home basedir
							...
							(matcher
							 ...
							 (lambda (x) (string-suffix-p "-notes.org" x)))
							(make-pair
							 ;; create a pair with the directory name & filename
							 (lambda (x) (cons (f-base (f-dirname x)) x)))
							)
					(seq-map make-pair
									 (seq-map remove-prefix
														(f-files projects-home matcher t)))))
	;; sort ?
...
#+end_src
** vterm
vterm requires libtool & [[https://cmake.org/][cmake]] install them with
#+begin_src sh
...
#+begin_example emacs-lisp
	(use-package drools-mode
		:mode "\\.drl\\'"
		:load-path "site-lisp/drools-mode"
		:custom
		(drools-tab-indent 2 "set indent size to 2")
		(indent-tabs-mode nil "do not use tabs for indent")
		:hook yas-minor-mode)
#+end_example
*** Old way
#+begin_example emacs-lisp
...
#+begin_src emacs-lisp
	(add-to-list
	 'load-path
	 (expand-file-name "site-lisp/fzf" user-emacs-directory))
	(require 'fzf)
	;; (eval-after-load "fzf"
...
#+begin_src emacs-lisp
 (message "Loaded all from %s" (ff-basename (buffer-file-name)))
#+end_src
