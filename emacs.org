...
#+begin_src emacs-lisp
  (use-package hydra
  	...
  	(defhydra common-org-files (:exit t)
  		"goto"
  		("a" (find-file (expand-file-name "../profile/asset.org" daily-home)) "asset.org")
...
#+begin_src emacs-lisp
  (defun package-repo-install (package-name package-url &optional branch load-filepath)
...
#+begin_src emacs-lisp
  (package-repo-install 'plantuml-mode "https://github.com/xshyamx/simple-plantuml-mode")
...
#+begin_src emacs-lisp
  (package-repo-install 'graphviz-dot-mode "https://github.com/xshyamx/graphviz-dot-mode")
...
** OpenAPI
#+begin_src emacs-lisp
  (package-repo-install 'openapi-mode "https://github.com/xshyamx/openapi-mode")
...
#+begin_src emacs-lisp
  (package-repo-install 'xmlstarlet-mode "https://github.com/xshyamx/xmlstarlet-mode")
...
Requires [[JQ Mode]]
#+begin_src emacs-lisp
  (package-repo-install 'restclient-mode "https://github.com/xshyamx/restclient-mode")
  (use-package restclient-mode
  	:load-path "site-lisp/restclient-mode")
#+end_src
#+begin_example emacs-lisp
  (unless (package-installed-p 'restclient)
    (package-vc-install 'restclient nil "https://github.com/pashky/restclient.el"))
  ;; (add-to-list
  ;;  'load-path
...
#+begin_src emacs-lisp
	(require 'cl)
	(package-repo-install 'git-commit "https://github.com/xshyamx/git-commit-mode")
	...
	(with-eval-after-load "compat"
		(require 'compat)
		(package-repo-install 'git-modes "https://github.com/magit/git-modes")
  								 ...
  								 (file+function ,filename add-daily-note)
  									,(string-join
  										`("** Interviews"
  										 "*** %^{Candidate Name}"
  										 "- %?"
  										 ,(concat "**** %\\1 / Interview notes" (make-string 24 ? ) ":noexport:")
  ...
  (defun org-reset-meeting-dates ()
  	...
  	(save-excursion
  		(save-restriction
  			...
  			(let ((day-contents (buffer-substring-no-properties (point-min) (point-max))))
  			...
  			(if (and
  					...
  					(let* ((content
  						...
  						(with-temp-buffer
  							...
  							(insert content)
  							(goto-char (point-min))
  							(when (re-search-forward (rx bol (+ "*") (+ space) (*? any) ":noexport:") nil t)
...
#+begin_src emacs-lisp
	...
	(defun camel-case-region (begin end)
		...
		(interactive "r")
		(when (use-region-p)
			(let ((repl (camel-case (buffer-substring-no-properties begin end))))
				(delete-region begin end)
				(push-mark)
				(insert repl))))
#+end_src
...
#+begin_src emacs-lisp
	...
	(defun kebab-case-region (begin end)
		...
		(interactive "r")
		(when (use-region-p)
			(let ((repl (kebab-case (buffer-substring-no-properties begin end))))
				(delete-region begin end)
				(push-mark)
				(insert repl))))
#+end_src
