...
#+begin_src emacs-lisp
	...
	(defun fc/daily-notes-matcher (path)
		...
		(and (string-prefix-p (concat daily-home "/daily-log-") path) (string-suffix-p ".org" path)))
	...
	(if daily-home
			(seq-map #'fc/daily-make-pair
				 (directory-files daily-home nil "daily-log-[[:digit:]]\\{4\\}\\.org$")
				 )
		nil))
...
#+begin_src emacs-lisp
  ...
  (package-repo-install
   'drools-mode
   "https://github.com/pdorrell/rules-editing-mode"
   nil
   "drools-mode.el")
...
#+end_src
** Open corresponding plantuml/grapvhiz source file if available
#+begin_src emacs-lisp
	(defun org-find-image-src ()
		...
		(let* ((el (org-element-context (org-element-at-point)))
			...
			(when (memq type '(link paragraph))
				(when (and (eq type 'paragraph)
				...
				(setq img (org-element-property :path el)
							plantuml-file (concat (file-name-sans-extension img) ".plantuml")
							graphviz-file (concat (file-name-sans-extension img) ".gv"))
				(when (string= "png" (file-name-extension img))
					(dolist (file (list plantuml-file graphviz-file))
...
#+begin_src emacs-lisp
	(defun org-reset-day (bc)
		(with-temp-buffer
			...
			(goto-char 0)
			(while (re-search-forward
							(rx bol (repeat 3 "*") (+ space) (? "DONE" eow (+ space))) nil t)
				(replace-match "*** TODO "))
...
Can be replaced with [[help:base64-decode-string][base64-decode-string]] with 2nd argument
...
#+end_src
* Join selected lines with separator
#+begin_src emacs-lisp
