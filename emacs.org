...
#+end_src
** Open emacs lisp sources in ~outline-minor-mode~
#+begin_src emacs-lisp
	(defun elisp-setup-outline-minor-mode ()
		(outline-minor-mode)
		;; show only outline
		(outline-hide-body)
		;; M-[pn] to hide & show subtrees
		(local-set-key (kbd "M-p") #'outline-hide-subtree)
		(local-set-key (kbd "M-n") #'outline-show-subtree)
		)
...
#+end_src
** Keybindings for deleting src blocks & subtrees
#+begin_src emacs-lisp
	(defun org-delete-block ()
		"Deletecurrent block."
		(interactive)
	...
	(defun org-delete-subtree (&optional element)
		...
		(let* ((heading (org-element-lineage
					...
					(if (and heading end)
							(delete-region (org-element-property :begin heading)
																(if (= end (point-max))
																		end (1- end)))
						(signal 'outline-before-first-heading nil))))

	(define-key org-mode-map (kbd "C-c C-v C-k") #'org-delete-block)
	(define-key org-mode-map (kbd "C-c C-k") #'org-delete-subtree)
#+end_src
* Utility Functions
...
#+begin_src emacs-lisp
	...
	(define-key org-mode-map (kbd "C-c f") 'ff-link-org)
	(setenv "FIREFOX_PROFILE" "/Users/shyam/Library/Application Support/Firefox/Profiles/sz5s61iv.default-release")
#+end_src
...
#+begin_src emacs-lisp
	(when (not (eq plantuml-output-type "png"))
