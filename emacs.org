...
#+begin_src emacs-lisp
	(defun eval-buffer-notify ()
		(interactive)
		(eval-buffer)
		(message "Evaluated buffer: %s" (propertize (buffer-name (current-buffer)) 'face 'font-lock-function-name-face)))

	(dolist (mode-map '(emacs-lisp-mode-map lisp-interaction-mode-map))
		(define-key (symbol-value mode-map) (kbd "C-c C-c") #'eval-buffer-notify))
#+end_src
* Finally
