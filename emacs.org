...
#+end_src
** Reset the dates on a meeting list heading
To use in project notes
#+begin_src emacs-lisp
  (defun org-reset-day (bc)
    (with-temp-buffer
      (insert bc)
...
#+end_src
** Remove meeting details under current subtree
To use in project notes
#+begin_src emacs-lisp
  (defun org-remove-meeting-details ()
    (interactive)
    (save-excursion
      (save-restriction
  	...
  	(org-cut-subtree)))))
#+end_src
** Add via hook to local keymap
#+begin_src emacs-lisp
  (add-hook
   'project-notes-file-hook
   (lambda ()
     (keymap-local-set "C-c r m" #'org-reset-meeting-dates)
     (keymap-local-set "C-c r d" #'org-remove-meeting-details)))
#+end_src
...
#+begin_src emacs-lisp
  ...
  (defun camel-case-region (begin end)
...
#+begin_src emacs-lisp
  ...
  hyphens"
    (string-join (split-string (downcase s) "[ \f\t\n\r\v_-]+") "-"))
