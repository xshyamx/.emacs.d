...
#+begin_src emacs-lisp
  (use-package hydra
    ...
    (defhydra common-org-files (:exit t)
      ...
      ("i" (find-file (locate-user-emacs-file "init.el")) "init.el")
...
#+begin_src emacs-lisp
  ...
  (defun jsp-file-options ()
    (when (string-match (rx ".jsp" eol) (buffer-file-name))
      (setq tab-width 2)))
...
#+begin_src emacs-lisp
  (defun m2-jar (groupId artifactId version)
    (let ((m2-repo (getenv "M2_REPO"))
  	(groupPath (replace-regexp-in-string (rx ".") "/" groupId))
  	(jar))
      (setq jar (expand-file-name
  ...
  (use-package eglot-java
    :ensure t
    :config
    (setq eglot-java-java-home (expand-file-name ".sdkman/candidates/java/24-tem" (getenv "HOME"))
  	eglot-java-java-program (expand-file-name "bin/java" eglot-java-java-home)
  	egolt-java-eclipse-jdt-cache-directory (no-littering-expand-var-file-name "jdtls-cache"))
...
#+begin_example emacs-lisp
  (load-file
   (locate-user-emacs-file
    "site-lisp/dbdiagram-mode/dbdiagram-mode.el"))
#+end_example
** gedcom
