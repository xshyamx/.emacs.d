(defconst work-holidays-2025
  '(((1 14 2025) "Pongal")
    ((3 30 2025) "Gudi Padva / Ugadi")
    ((3 31 2025) "Ramzan / Id-ul-Fitr")
    ((4 14 2025) "Dr. Ambedkar Jayanti / Tamil New year")
    ((8 27 2025) "Vinayaka Chaturthi")
    ((10 20 2025) "Deepavali"))
  "List of holidays in 2025")

(defconst work-holidays-2024
  '(((4 9 2024) "Gudi Padva / Ugadi")
    ((4 11 2024) "Id-ul-Fitr / Ramzan")
    ((10 31 2024) "Deepavali"))
  "List of holidays in 2024")

(defconst work-holidays-2023
  '(((3 22 2023) "Gudi Padva / Ugadi")
    ((6 29 2023) "Id-ul-Zuha/ Bakrid")
    ((9 19 2023) "Ganesh Chaturthi")
    ((10 24 2023) "Dussehra / Vijaya Dashmi"))
  "List of holidays in 2023")

(defconst work-holidays-2022
  '(((1 14 2022) "Pongal")
    ((4 14 2022) "Dr. Ambedkar Jayanti")
    ((4 15 2022) "Good Friday")
    ((5 3 2022) "Ramzan/Id-ul-Fitr")
    ((8 31 2022) "Ganesh Chaturthi")
    ((10 5 2022) "Dussehra / Vijaya Dashami")
    ((10 24 2022) "Diwali"))
  "List of holidays in 2022")

(defconst work-holidays-2020
  '(((1 15 2020) "Pongal")
    ((3 10 2020) "Holi")
    ((3 25 2020) "Ugadi")
    ((5 25 2020) "Ramzan/Id-ul-Fitr")
    ((8 22 2020) "Ganesh Chaturthi")
    ((10 26 2020) "Dussehra / Vijaya Dashami")
    ((11 14 2020) "Diwali"))
  "List of holidays in 2020")

(defconst work-holidays-2019
  '(((6 5 2019) "Ramzan (Id-ul-Fitr)")
    ((9 2 2019) "Ganesh Chaturthi")
    ((10 8 2019) "Dussehra / Vijaya Dashami")
    ((10 28 2019) "Diwali"))
  "List of holidays in 2019")

(defconst work-holidays-2018
  '(((6 15 2018) "Ramzan (Id-ul-Fitr)")
    ((10 19 2018) "Dussehra / Vijaya Dashami")
    ((11 6 2018) "Diwali"))
  "List of holidays in 2018")

(defconst work-holidays-2017
  '(((3 29 2017) "Ugadi / Gudi Padwa")
    ((6 26 2017) "Ramzan (Id-ul-Fitr)")
    ((8 25 2017) "Ganesh Chaturthi")
    ((10 19 2017) "Diwali (Laxmi Puja)"))
  "List of holidays in 2017")
