...
;;; Code:
(defun project-notes--reset-day (bc)
  (with-temp-buffer
    (insert bc)
    ...
    (goto-char 0)
    (while (re-search-forward
  	  (rx bol (repeat 3 "*") (+ space) (? (or "TODO" "DONE") eow (+ space))) nil t)
      (replace-match "*** TODO "))
...
(defun project-notes-reset-meeting-dates ()
  (interactive)
  (save-excursion
    (save-restriction
...
(defun project-notes--yank-meeting-item (password)
  (let ((entry (substring-no-properties (org-get-entry)))
	...
	(message "Copied meeting %s" (if password "password" "number"))))))
...
(defvar project-notes-mode-map
  (let ((keymap (make-sparse-keymap)))
    (keymap-set keymap "C-c r n" #'project-notes-yank-meeting-number)
    (keymap-set keymap "C-c r p" #'project-notes-yank-meeting-password)
    (keymap-set keymap "C-c r m" #'project-notes-reset-meeting-dates)
    (keymap-set keymap "C-c r d" #'project-notes-delete-meeting-details)
    keymap)
